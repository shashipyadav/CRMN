...
public class BrochureAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public BrochureAdapter(Activity activity, List<BrochureModel> amenitiesModelList, SharedPrefManager prefManager) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ((VContentInfoHolder) holder).title.setText(amenitiesModelList.get(position).getBrochuredesc());


        final String imageUrl = prefManager.getClientServerUrl()+ "uploads" +prefManager.getCloudCode() + "/Folder3567/" + amenitiesModelList.get(position).getBrochureimage();
        String someFilepath = amenitiesModelList.get(position).getBrochureimage();
       ...
       /* ((VContentInfoHolder) holder).iv_image.setOnClickListener(new View.OnClickListener() {
        ...
        ((VContentInfoHolder) holder).iv_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                request.setTitle(amenitiesModelList.get(position).getBrochuredesc()+extension);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    request.allowScanningByMediaScanner();
                    request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
                }
                request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, amenitiesModelList.get(position).getBrochuredesc()+extension);
