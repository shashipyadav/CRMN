...
public class PaymentPlanAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public List<PaymentPlanModel> amenitiesModelList;
    private SharedPrefManager prefManager;

    public PaymentPlanAdapter(Activity activity, List<PaymentPlanModel> amenitiesModelList, SharedPrefManager prefManager) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        ((VContentInfoHolder) holder).title.setText(amenitiesModelList.get(position).getPaymentdesc());



        final String imageUrl = prefManager.getClientServerUrl()+ "uploads" +prefManager.getCloudCode() + "/Folder3566/" + amenitiesModelList.get(position).getPaymentimage();
        String someFilepath = amenitiesModelList.get(position).getPaymentimage();
       ...
       /* ((VContentInfoHolder) holder).iv_image.setOnClickListener(new View.OnClickListener() {
        ...
        ((VContentInfoHolder) holder).iv_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                request.setTitle(amenitiesModelList.get(position).getPaymentdesc()+extension);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    request.allowScanningByMediaScanner();
                    request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
                }
                request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, amenitiesModelList.get(position).getPaymentdesc()+extension);
